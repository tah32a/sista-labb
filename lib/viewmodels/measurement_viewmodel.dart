...
/// ViewModel managing measurement state and coordinating services
class MeasurementViewModel extends ChangeNotifier {
  ...
  void startMeasurement() {
    ...
    // Clear previous data and reset algorithms
    _measurements.clear();
    ...
    // Subscribe to sensor stream and process data
    _sensorSubscription = _sensorService.sensorStream.listen(_processSensorReading);
  ...
  /// Process incoming sensor reading through both algorithms
  void _processSensorReading(SensorReading reading) {
    // Compute angles using both algorithms
    ...
    // Notify UI for live update
    notifyListeners();
  }

  /// Export measurements to CSV file
  /// Returns the file path or null if export fails
  ...
  /// Configure EWMA alpha parameter for Algorithm 1
